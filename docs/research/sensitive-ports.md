...
> **Amended by §15** ([#76](https://github.com/winniel123/verge-asm/issues/76)). **The seven listed
...
> (§15.3). **The weak tier grows to three on a different row** — `10255/tcp` kubelet, whose owner
> states no network position and whose shipped config API carries `readOnlyPort` *"Default: 0
> (disabled)"* (§15.5). **No `(port, transport)` pair moves and no row moves.**
>
> | Footing | Pairs |
> |---|---|
...
>
> **Two cells are thin and are flagged rather than smoothed** (§15.9). `10250`'s scoping rests on a
...
> cites for it, is a **non-owner's** under §10.5 and corroborates only (§15.6). Read §15.9 before
> quoting either cell.
...
> **Amended by §15** ([#76](https://github.com/winniel123/verge-asm/issues/76)). **Four rows in this
...
> above stand and §15 says what each is now doing.
>
...
> > — `THREAT_MODEL.md`, [etcd-io/etcd](https://github.com/etcd-io/etcd/blob/v3.7.1/THREAT_MODEL.md) `v3.7.1` (present at `v3.7.0`; **absent at `v3.6.14` and `v3.5.33`** — §15.9)
>
...
> `25672`, which is RabbitMQ's own port, and **corroborates only** for `4369` (§10.5, §12(c), §15.6).
...
>
> Neither Erlang/OTP sentence names `4369`, which is why §15.9 flags this as the table's thinnest cell.
...
> **Discharged by §15** ([#76](https://github.com/winniel123/verge-asm/issues/76)). **All seven are
...
> `readOnlyPort` *"Default: 0 (disabled)"* (§15.5). `9300` joins the prohibition tier and `2375`,
> `4369` and `10250` the scoping tier. **No `(port, transport)` pair moves**, so #12 stays unblocked by
> this line — but §15.5's by-catch, that §3.4's kubelet defaults are quoted from a generated page the
...
## 15. The footing table's seven uncovered rows are placed, and it now states its own coverage
...
text stands and is marked, per the name-and-withdraw convention, and where §15 and an earlier section
disagree, **§15 governs**.
...
> contradicts** (§15.5), and `4369/tcp`'s attestation is a **non-owner's** (§15.6).

### 15.1 What was retrieved
...
| Kubernetes | `staging/src/k8s.io/kubelet/config/v1beta1/types.go`, `pkg/kubelet/apis/config/v1beta1/defaults.go` — `kubernetes/kubernetes` `v1.34.1` | `10255`, and §15.5's by-catch |
...
| Erlang/OTP | `erts/doc/references/epmd_cmd.md`, `system/doc/reference_manual/distributed.md` — `erlang/otp` `OTP-29.0.5` | `4369`, and §15.6's by-catch |

`2375/tcp` Docker needed no retrieval: the sentence is already in §3.4, cited for its cell-mate.

### 15.2 The two clerical placements
...
reason for it is not** — see §15.6, which is why this row took a retrieval after all.

### 15.3 `2379` and `2380` etcd — the ticket's crux, and it resolves the other way
...
at `v3.7.1`; this section reads the same tag, which is the consistent choice. §15.9 flags what that
costs.

### 15.4 `9300` Elasticsearch — the *node* sentence does reach the transport port, and the owner says so
...
### 15.5 `10250` and `10255` kubelet — one to scoping, one to the weak tier
...
what it is **not** doing for etcd in §15.3. **[measured]** `ports-and-protocols.md` at
...
### 15.6 `4369` epmd — the placement §13.7 called clerical, and the reason it is not
...
permissive and **silent** under §10.4. §15.9 flags how thin this is.
...
### 15.7 The ruling — the table restated, with its coverage in the table
...
**What changed and what did not.** Two placements are clerical (§15.2), one confirms a reading §13.7
...
### 15.8 Every dependent figure, checked rather than asserted
...
§15.6's per-port ownership rule is a statement of what §10.5 already requires rather than a new
decision.

### 15.9 Thin ground, flagged per the standing rule
...
### 15.10 Retrieval method and hazards, recorded per §9.5, §11.9, §12.9 and §13.10
...
- **The `.sample` trap from §13.6 has a sibling in file *purpose*, and it was met.** etcd's
  ...
  §15.3 rules on the sentence and records the purpose argument in full rather than suppressing it.

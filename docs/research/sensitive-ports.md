...
> **Amended by §18** ([#87](https://github.com/winniel123/verge-asm/issues/87)). **No entry is added or
...
> names Kibana directly. **The restated cells are in §18.6, with the reopening criteria.** **No
...
11. ~~**Has [ADR-0042](../adr/0042-a-squat-is-contested-where-the-other-convention-is-live.md)'s
    liveness test ever been run on `esmagent` and `fmtp`?**~~ **Closed by §18**
...
> **Amended by §18** ([#87](https://github.com/winniel123/verge-asm/issues/87)). **Rows 11 and 12 both
...
## 18. ADR-0042's liveness test, run on `esmagent` and `fmtp` — both registrations are live
...
### 18.2 `esmagent` is live — the registration is followed to its registrant, and the registrant's successor still ships on the number
...
### 18.3 The limb-3 check on `5601`, and it disposes of the *other* competitor
...
### 18.4 `fmtp` is live — the specification names the number in a `shall`, and its owner's 2026 catalogue still lists it
...
and it is the reopening criterion in §18.6 rather than a footnote.
...
### 18.5 The two residue rows were never the same shape, and §17.1 row 11 mis-scored one of them
...
in §18.2. **Also withdrawn as a transposition risk**, restating what §4.6 already flagged: Elastic's
...
### 18.6 The ruling — §4.6's two cells restated with their defeating artefacts
...
### 18.7 Every dependent figure, walked rather than asserted
...
| §2.6's boast | true of all 37 | **unchanged** — no row's attestation is touched |
| §4.5 *the list's weakest row* | 5432/tcp | **unchanged** |
| §4.6 exclusions | 18 named | **18 named, unchanged.** No entry is added or removed; **two have their stated reason restated** with cited artefacts (§18.6), and one clause of the `5601` cell is **withdrawn as a ground** (§18.5) |
| §6.1 containment arithmetic | 28 + 4 + 5 = 37 | **unchanged** — no member enters the sensitive half, so no containment gap can be created |
| §17.1 row 11, `5601`, *Sole ground?* | **Partly** | **No.** §2.1's closed claim set refuses the row by name; the negative was never exposed (§18.5) |
...
| §17.8's residue | two rows, *"§2.4's backlog, not §2.2's"* | **discharged, and the backlog is finished for §2.4 as well** (§18.8) |
...
### 18.8 The residue goes to zero, and what that does to §17.8's *fixed point*
...
footing has already moved once (§18.4).
...
### 18.9 Thin ground, flagged per the standing rule
...
**already** lapsed (§18.4). Two things hold it: the catalogue is the owner's own current document and
...
**§18.5's re-scoring of §17.1 row 11 is a re-reading, not a retrieval, and it does not move a row.**
...
### 18.10 Retrieval method and hazards, recorded per §9.5, §11.9, §12.9, §13.10, §14.6 and §17.10
...
- EUR-Lex, retrieved because it could only weaken the finding, and did (§18.10): [CELEX `32007R0633`](https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX%3A32007R0633) — *"No longer in force, Date of end of validity: 04/10/2023; Repealed by 32023R1771"* · [CELEX `32023R1771`](https://eur-lex.europa.eu/legal-content/EN/TXT/HTML/?uri=CELEX:32023R1771), which folds ATM/ANS systems requirements into [Implementing Regulation (EU) 2017/373](https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:32017R0373), whose consolidated text does not name FMTP. **It carries the finding in neither direction** — limb 4 keys currency to the party, not to a legislature — and it is `8500`'s reopening pressure (§18.6)

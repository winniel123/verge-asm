...
- `examples/` — **the console's original design reference** ([ADR-1988](../adr/1988-design-system-examples-is-the-consoles-original-design-reference-and-it-binds-no-template.md)): `examples/console/` (the console screens + `ConsoleApp.jsx` shell + a screen README), plus `examples/Homepage.jsx` (marketing) and `examples/DocsPage.jsx` (docs). They record what each screen looked like when the T0 port consumed them. **They bind no template, no ticket and no session.** A console screen's information architecture is decided in its SPEC under `docs/spec/`, and the served `templates/*.tmpl` realises it. Read an example for the original intent behind a screen. A difference between an example and its template is not a defect, and no template change obliges a matching example change.
...
- **Drift is the thesis.** Drift is the thing this product exists to do. It is never a secondary screen. Both the console example and the served templates keep Drift a top-level nav item. [ADR-1988](../adr/1988-design-system-examples-is-the-consoles-original-design-reference-and-it-binds-no-template.md) retires the verbatim-port obligation, and this guardrail does not travel with it: it binds every console surface directly. Never let a dashboard-first reading demote drift.

...
| **`suspect-firewall` port threshold** (100) | **REFUSED — there is no such rule.** The v1 set is **sixteen** ([ADR-0024](../adr/0024-a-rules-domain-is-the-extension-of-its-name.md)) and `suspect-firewall` is not among them; §6.3's suppression heuristic never entered the model. A threshold for a rule that does not exist is not a knob |

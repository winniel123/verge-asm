...
| 15 | `dts-field-prose` | A `.d.ts` block whose next code token opens a property key. §6.6 states the predicate. |
| 16 | `short-label` | One line, six words or fewer. |
| 17 | `prose-other` | Everything else. |
...
**The search surface is every swept package, not the ticket's own directory.** A `cmd/web` test
drives `internal/` code, so a keeper for it may sit anywhere the sweep has reached. This was the
dominant review defect in #1223, ahead of the wrong citation §7.7 names as dominant. That ticket
grepped `cmd/web` production exhaustively and stopped, because §7.4 frames the collapse as a
`cmd/web` fact. Both misses sat outside: `internal/queue/seedwithdrawal.go` and `cmd/web/flash.go`'s
own package. #1225 collapsed onto three `internal/` packages and #1226 onto three more.

**With a batch open, "merged" is not the whole of what you must search.** A keeper on `main` today
can be a deletion in an open sibling PR. `Table.jsx:4-5` states the clipped-cell rule on `main`, and
PR #1403 deletes it. #1405 read #1403's branch before choosing, and rested on `Table.d.ts:11`
instead, which §4.3 keeps and which both trees hold. This is the mirror of §7.5 rule 7: a sibling
PR's **claim** is not evidence, and its **code** is not stable evidence either.
...
**Two trees the rule names outright.** `internal/db` reads as swept `internal/` production and is
not. `querier.go`'s doc comments **are** `db/queries/*.sql`'s comments regenerated by `sqlc`, so a
D2 sweep moves them (#1226). `db/migrations` is the stronger case, because §1.4 excludes it
permanently rather than merely leaving it unswept. It states rules a D2 agent wants — `24700:64-66`,
`19000:52` and `22100:10` — and #1227 reports a real pull toward collapsing onto it.

**A swept sibling's silence is not a keeper either.** This rule runs in a second direction and the
SPEC named only the first. #1233 deleted the GitHub workflow-command grammar from
`doclint/github.mjs` on the ground that the swept Go twin, `cmd/commentlint/github.go`, keeps
nothing. Both review axes called it the branch's strongest over-delete, and the block is restored.
...
**The test has a second cut, and a web-asset ticket needs it.** Keep the rule whose wrong edit is a
non-design refactor inside the file. Collapse the rule whose wrong edit is a design choice, because
`CLAUDE.md` routes every design session through the design documents first (#1231). The first cut
asks what the document is for. This one asks what the wrong edit would be.

**The document search must reach a per-component authoring doc.** `design-system/components/display/`
alone holds **51** `<Component>.prompt.md` files, and `docs/agents/design-system.md` orders a session
to read one before it uses that component. #1231 reports that corpus dissolved 45 of its 67 `.jsx`
files. #1232's first draft kept two survivors those files already state verbatim. Neither §4.1 nor
§8.3 named this shape before batch seventeen.
...
unavailable. `cmd/web/rawoutput_test.go` carried **six** bare references — `spec §6`, `§1.2`,
`spec §1.2`, `§1.3`, `spec §1.3` and `spec §5.2` — and no block named `raw-job-output.md` (#1223).
The bare word "spec" did the work. **Where no block names the document, a survivor keeping a bare
`§n` adds the name.** It does not hunt for a naming block to keep.

**A quoted phrase is a reference, and no citation check sees it.** The two orphan rules above both
test a `§` or a file name. `tagging.mjs:68` quoted the phrase `lives here once` from `engine.mjs`.
#1233 deleted that phrase in PR #1397, so the quotation dangled on `main` before #1232 ran. A
quotation carries no citation shape. **Grep a distinctive phrase of the block you delete**, not only
its file name.
...
**Reason 2 fails on a file the diff will not render, and two NUL bytes are enough.**
`docs-site/scripts/doclint/engine.mjs` holds **2** NUL bytes at one site, inside the dedup key
`` `${rule.name}\x00${hit.line}\x00${hit.message}` `` (#1233). `file` then calls it `data`. `git
diff` renders it as `Bin`. A grep wrapper returns silent empty output with no error. It is #1233's
largest file at 180 comment lines, and one of that ticket's own early greps skipped it that way.
**A reviewer needs `git diff --text`, and every search of such a file needs `grep -a`.** Say so in
the PR body. The reviewer cannot see the deletions this rule promises them.

**The measurement of that file was wrong twice before it was right.** The orchestrator's first
figure was 301 NUL bytes, an artefact of `grep -c $'\x00'`, which bash truncates to an empty pattern
that matches every line. #1234 caught it, and three independent methods give 2. **A plausible figure
reached for a reason unrelated to what you measured is the failure this map keeps recording.**
...
| `dts-field-prose` | ~230 | **Keep.** Carve-out. |
...
**`dts-field-prose`.** A `.d.ts` file has no implementation. Its field prose **is** the interface,
...
**The class id is `dts-field-prose`, and it is one name across four places.** #1406 added the class
to `commentlint`, so the taxonomy (§2.1), this ruling, the residue manifest and §6.6's rule ids all
say the same word. The SPEC called it "`.d.ts` field prose" through batch seventeen, which is prose
and not an id. §6.6 states the predicate that decides it.

**The carve-out is the field prose, not the file.** A `.d.ts` block that sits above an
`export interface` rather than above a field is an ordinary comment. #1232's two `@startingPoint`
...
carries a body below the block. #1233, #1234 and #1235 each deleted every `@type`, `@typedef`,
`@param` and `@returns` in their files, reaching that independently. §2.3's JS and TS protected row
omits every JSDoc tag, and no tool here reads one.

**Both carve-outs bind the sweep only.** `package-doc` at a 3-line cap and `dts-field-prose` are
...
4. It holds 100 columns or fewer. **The measure is runes across the whole physical line, after
   `gofmt -w`, with a tab counted as 4 columns.** The whole line, not the comment text alone.
   `gofmt` aligns a trailing comment to the widest field in its run, so a survivor that fits when
   written can overrun after formatting. `internal/auth/session.go`'s `Token` field reached 105
   that way (#1166).

**The measure is stated here because the alternatives are not close.** #1224's two review axes each
...
already rules that for a comment. It applies to the SPEC itself, and to every falsifier or example
this SPEC takes from a live file. A pointer into a file the sweep has not reached is a pointer the
sweep is scheduled to break.

**An amendment-bearing ADR is cited by its amendment, never by a section, and a `§n` against one is
presumed wrong.** The form at the head of this section reads `§x.y` as the primary locator and
...
those, a `§n` is wrong by construction, and a grep answers before any reading starts. Four repair
routes cover every case, and eleven repairs in this session's defect PR settled them:

| What states the rule | The repair |
| --- | --- |
| The cited ADR itself | Drop to the bare `ADR-nnnn` form. `(ADR-0021 §4.4)` became `(ADR-0021)` in two `Version` consts |
| Another document | Name that file. `(§8.5)` became `golden-corpus.md §8.5`, and `(ADR-0005, §2.2)` became `passive-discovery-sources.md §2.2` |
| Nothing | Delete the citation and keep the reason clause. `(ADR-0005, ADR-0027 §7)` became `(ADR-0005)` |

**A bare `§n` beside an ADR reference must never appear.** It is unrepairable by inspection, because
a reader cannot tell an ADR section from a spec section from an omission. The three routes above
...
**A compression that de-hyphenates a term of art inverts it.** ADR-0105's `count-of-subjects` names
an estate total. "count of subjects" does not, and `Inventory.jsx:149` and `:171` contradict the
open reading 136 lines below (#1231). A compression is where this happens, because the agent is
rewriting the term rather than copying it. **Copy a hyphenated term of art verbatim.**
...
2. **Trailing on the declaration line**, where the reason is about the declaration itself. The line
   must still meet §4.4's 100-column cap, measured after `gofmt -w`, and its six-word floor where
...
100-column cap has no room for a trailing comment either. `internal/measure/useragent.go` is the
...
100-column cap, so placement 2 fails. Placement 3 was the only legal option.
...
**On a table-driven render test the tell kills a whole docstring at once.** Rule 1 is written for a
single assertion. These files are built on `for _, want := range []string{…}` with a
`t.Errorf("… missing %q; body: %s", want, page)`, and the docstring above lists the same strings in
prose. One tell therefore kills the block rather than one sentence. The trailing label inside the
`want` list is the same tell in miniature. **143 of #1221's 148 baseline `lint` flags were these two
shapes**, split 73 `go-decl-comment` and 70 `short-label`. Read the loop's message, not the
assertion.

Rule 3 is load-bearing. An uncited WHY inside a test file explains a classification decision, and an
absolute citation requirement would delete it. §4.7 keeps it, and §4.7 wins.

**Rule 3's old example did not survive rule 3, and that is the finding.** It named a block in
`cmd/web/adr0130_contract_test.go` by line. Stage B moved the block, then #1220 deleted it on gate
A, and the spec review reached the same verdict independently. The reason the deletion is correct
also explains the rule: the block's uncited prose restated the `map[string]string` beneath it, so
§4.1 killed it before rule 3 could keep anything. **Rule 3 keeps an uncited reason. It does not keep
an uncited restatement**, and only §4.1 tells the two apart. Read rule 3 with the §4.6 rule 1 tell
below, which is what decides the common case.

**A `map[string]string` whose values are prose fails gate A for every comment above it.** The data
literal stands where rule 1's `t.Errorf` message stands, so the claim is recoverable from the
declaration. Seven blocks in #1220 died that way. This is the same tell with a different carrier.
...
**This list must never filter §4.1's orphan grep.** The two checks ask different questions. This one
asks *may I repair a citation to this file?* §4.1 asks *does this file point at what I am deleting?*
The answers differ. #1222 filtered `docs/spec/comment-policy.md` and `docs/research/comment-*` out of
its `docs/` orphan grep on this section's authority, and missed a rotted pointer in this SPEC as a
result. Its standards review caught it. #1225 obeyed the corrected rule and turned up
`docs/spec/release-pipeline.md`'s `credflow_sessions_test.go` pointer the same way.

**A claim about a file the ticket does not own is a lead, never a finding.** #1233 reported
`DocsPage.jsx:14`'s `docs/DOCS-IA.md` as naming a file that does not exist. **It resolves.**
`DocsPage.jsx` sits at `design-system/examples/`, and `design-system/docs/DOCS-IA.md` is on disk at
2,930 bytes. The orchestrator corrected it before it reached #1231, which owns that file. This is
§7.5 rule 9 inside a PR body rather than inside an issue body.
...
4. **The section rules, rather than forwarding to the rule.** ADR-0129's #954 amendment states the
   ...
   **Three more non-resolving shapes join forwarding, and each passes every earlier test.** A
   section that **explicitly declines to rule**: `#1046` is live, closed and on topic, and its body
   states the exact rule the comment gives under a heading reading `## Not decided` (#1222). A
   section that **explicitly defers**: `#351` says "define … whether pre-release tags publish"
   (#1233). And a citation that **defers to a closed issue**, twice in #1234 — `#821` and the
   `Tv/#351` family. The last is the cheapest to detect, because reading the issue's state answers
   it. #1221's finding 7 named the uncited deferral, and these are its cited siblings.

   **A citation forward decides which of two inherited citations to keep.** ADR-0105 rules the
   denominator in its own voice and forwards to ADR-0072. #1231 first kept the forwarded-to citation
   and dropped the ruling one, and its review inverted that. **Keep the citation to the section that
   rules.**
...
resolve at `ADR-0124:56-57`, and resolves nowhere itself (#1215). Batches 13 to 17 add `#21b`,
`#21c`, `#25a`, `#25f`, `AC#4`, and four documentation-tier letters — `D2` and `D6` cited to
`DocsPage.jsx`, which names `D3` and `D4` and neither of these, then `D3` and `D4` themselves once
`docs-site/PIPELINE.md` stopped stating them (#1233, #1231). #1236 adds a further 21
`SPEC-CHANGE #nn` collision ids, `#18` to `#27`, every one resolving to a live unrelated issue.

**`T1` and `T2` resolve falsely as documentation tiers** inside
`docs/spec/documentation-style-standard.md` (#1221). This is the `G2` shape sitting in the style
corpus rather than the ADR corpus. The token answers a grep and rules something else.

**A bare token beside a `#nnn` may be defined inside that issue's body, where no tree grep reaches
it.** `WORK-ORDER-DOGFOOD-R1` returns three tree hits, all of them the never-a-repair-target files
below. That is the exact fingerprint of a dead token, and the token is alive. **#633's body carries
a heading `Repo-side flags (WORK-ORDER-DOGFOOD-R1)`** whose item 3 is the wiring the comment
describes. #1230 filed it dead on the grep and corrected itself on reading the body. #1236 reached
the rule from the other side. `WORK-ORDER-FEATURES-1` carries no `#nnn`, and an issue search on the
bare token returns **#632**, whose body is the full rule.
...
**It has a benign twin, and the disagreement alone does not condemn the citation.** The rule above
reads as if it did. `#337` is the bug report saying the TOTP secret **is** stored in cleartext. The
rule in force is that report's fix, so the comment citing it is right (#1224). Ask which of the two
the citation names: the position the issue argues against, or the rule the issue produced.

**A token on this section's wrong-citation list is not wrong everywhere.** `#322` is measured wrong
in `cmd/web/clientip.go` above and **correct** in `cmd/web/ratelimit_test.go` (#1226). The list
...
`ADR-0129`, `ADR-0053`, `ADR-0081` and `ADR-0110` as a token deserving a second read wherever it
appears.
...
**A `false` verdict is an assertion, and nothing re-checks it.** The ledger is where a wrong `false`
lands, and a wrong `false` is invisible in the diff exactly as a wrong survivor is. One of #1226's
four `false` rows was wrong on the facts: the deletion stood on a §4.1 collapse and the row had to
be re-labelled. #1222's spec review reclassified **13** `false` verdicts to §4.1 or §4.6-rule-1
deletions, taking that ticket's count from 22 to 9. **A `false` row needs the same second read a
survivor gets.**

**Refusing a `false` verdict is the same act, and #1236 is the worked case.** Its 23
`Source of truth: …jsx` lines are **not** false. ADR-0110 is `Accepted` and holds the examples
authoritative, so the lines die on gate A with no `false` row. That ticket refused four such
...
**Eleven more falsifiers are measured. Only the first three are the product moving under the code.**
...
8. **The agent's own new survivor, against code it never touched.** Falsifier 7 is traceable
   ...
   measured. A survivor read `// No seed declares 10.0.0.0/8`, and `cmd/web/proposals_test.go`
...
9. **The state the claim describes is unreachable, so it was never true of a running product.**
   Every falsifier above describes a once-true claim (#1224). Nothing moved here, and nothing could.
10. **The commit that wrote the comment falsified it.** `simple-tenses.mjs` asserted a SPEC line
    that the same commit, `47d150d`, was changing (#1235). **The cheap check is `git log -S` on the
    cited string**, which puts the comment and the claim's last change in one output.
11. **The constraint never existed at all.** `measure.mjs:9` said "Run from the `docs-site`
    directory". #1232 disproved it by execution four ways, with byte-identical output from either
    working directory. Falsifier 10 is a claim that was true for the length of one commit. This one
    was never true, and this section had no word for it before batch seventeen.
...
**A skipped test is not a second signal here, and a skip stating its reason is not a defect.**
#1223 read `TestIntegrationsTabHidden`'s skip as silence about the drift. It is not. That skip is one
half of a deliberate pair of flag guards, and each states its reason. One skips because the
integrations surface is live and the tab must render. `skipIfIntegrationsHidden` skips the
behavioural tests because the surface is hidden (#388). Whichever way the constant is set, one set
runs and the other says why it did not. **The comment finding stands. The test finding does not**,
and no ticket was filed.

**A compression is an assertion, and a freshly compressed line reads as freshly checked.** Every
falsifier above describes a comment the world moved under. None describes one the sweep
...
**The tell binds a compression too, and compression makes the quantifier harder to check.** The
paragraph above is written for a clause deletion. **A compressed survivor inherits the deleted
block's quantifier, and the evidence justifying it goes out with the text.** Falsifier 8 fired three
times in #1223 on that shape alone.

- "the card and the history are this tab's **only** `st-table` users". `settings-scans` holds one.
- "these are the **two** orders /signals emits". Only one is verifiable.
- "the fold posts **no** field". Both forms in `inbox.tmpl` stamp it. Deleted rather than
  re-authored, per the verdict above.

#1221 measured the same shape from the other side. **A rewrite that narrows an inherited sentence
can promote a vague clause into a precise false one**, three times in one ticket. The agent did not
invent the claim. It sharpened someone else's.
...
`authors no markup`, `byte-exactness gate`, `screenshot diff`.

**The last two are batch thirteen's addition, and the six before them found nothing there.** 13 of
`cmd/web/devfixtures_test.go`'s 14 docstrings carried **both** of the new phrases and **none** of the
original six (#1222). Both halves are false: the tests are Go value comparisons rather than byte
compares, and no pixel or screenshot harness exists anywhere in the tree.

**A version token is a falsifier, and the artefact's own header is the check.** `rundetail_test.go`
claimed *"Package v3.17.0"* twice while `design-system/templates/rundetail.tmpl` declares
`package v3.8.0`, and `inventory_test.go` claimed `v3.2.4` against `v3.4.0` (#1221). Add
`package vN.N.N` to the detector list. One command answers it: read the template's own header.

**The phrase list did not catch the retired doctrine in a web-asset ticket. An absent artefact did**
(#1234). All 24 templates carry a `DESIGN-OWNED ARTIFACT … WORKFLOW.md v4` banner naming a file that
exists nowhere in the tree (#1236). **The tell that works there is `ls` the path a comment names.**
...
`v3.15.0 dogfood ss3`, `WORKFLOW.md`, and the whole `NN-console.jpg` reference-shot family rather
than two of its members. **A named CI gate counts too**: `cmd/web/templates_error.go`
claimed "CI gate G1 byte-compares it", and no such step exists in `.github/workflows/` (#1218).

**Scope the grep to the file the comment sits in.** A comment can name a helper that exists and that
its own file never calls. A tree-wide grep for the helper then answers `present` (#1222).
`cmd/web/scope_prg_test.go` said the scope tests read a refusal's message through the shared
`refusalPage` and `prgLanding` helpers. Both helpers exist. That file called neither, using
`getBody` throughout.
...
**Where a block is part-verifiable, look for the reading that makes it true before you delete it.**
§4.2 and this section both point at deletion, and neither asks that question. #1231 deleted
`Select.jsx:29` and its spec review correctly restored it. The entrance the comment names is a
**containing** panel's, not the panel's own. The tie-break is for a block you cannot decide, not for
one you have not finished reading.
...
**"Sound" grades the token substitution and not the newline, so `verify` is blind to ASI.** The JS
skeleton is a token stream with no newline record. #1235 measured the consequence. Rewriting
`return violations;` as `return`, a newline, then `violations;` changes the function from returning
the array to returning `undefined`, and `verify` reads **clean**. A one-character string change on
the adjacent line reads `changed`. **A trailing-comment deletion can produce exactly that line
break.** Read the `.d.ts`, `.ts` and `.mjs` row as sound against a substitution and blind to an
...
**`.jsx` carries the same hazard, and this rule read as `.tmpl`-only until batch sixteen.** A
`{/* … */}` sitting between two runs of literal JSX text is a **separator**. #1233 measured all
four shapes. Deleting the line merged `"On this page"` and `"— rendered table"` into one child and
read `changed`. Leaving `{}` on its own line reads `clean`. Blanking the line and stripping only the
comment bytes both read `changed`. `render.jsx` as shipped hits none of it, and #1231 and #1232 held
...
**The result is on `main`, and it is the shape a later reader meets.** PR #1396 deleted lines 1 to
11, which is exactly what the parser called the comment. The file now opens mid-sentence at
`, messages, delivery)` and carries an orphaned `*/}}` at its line 32. A second instance sits at the
old line 1249, where a `*/}}` was already followed on the same line by the fragment
`e store) */}}`. **This is correct behaviour under §5.4, and it looks like damage.** It renders
nowhere. Text outside every `{{define}}` belongs to a root template `cmd/web` never executes. Read
it as unreachable stale doctrine rather than a live defect, and say so in the PR body of any ticket
that owns such a file.

### 5.5 Two caveats

**The circularity caveat.** SQL and CSS shipped with no independent cross-check. The same hand lexer
both finds the comments and builds the skeleton. A lexer that misreads a comment misreads it
consistently, so the check still passes. This was the one unvalidated risk in the design, and the
CSS half is now closed below.
...
**The CSS half of the caveat is closed, and the check is cheap enough to require.** An earlier
version of this section said no independent parser reads this repo's CSS. `postcss`, `css-tree` and
`lightningcss` are all vendored in `docs-site/node_modules`. #1231 compared the `postcss`
declaration AST before and after on all 10 of its `.css` files and got byte-identical output.
**Require that comparison on any future CSS sweep.** The SQL half stands.

**An independent minifier can print a confident false pass, and the failure is silent.**
`esbuild --loader=jsx --minify <path>` **errors** with `"loader" without extension only applies when
reading from stdin`, because the flag applies to stdin alone. A `$(… | sha256sum)` comparison then
compares two hashes of the empty string, `e3b0c442…`, and reports the two trees identical. #1405 hit
this on its first run and caught it on the byte count rather than the hash. **Read the byte count
and the stderr size, never the hashes alone.** The sound form is `esbuild --loader=jsx --minify <
file`. This is the trap on the road the ASI blindness above sends an agent down.
...
| Surface | Delete rule | Forced or chosen |
| --- | --- | --- |
| Go | remove the block's own lines, then `gofmt` (§3.8) | Forced |
| `.tmpl` | delete the comment's byte range, leave its line (§5.4) | **Forced.** Two of three shapes fail `verify` |
| `.sql` | remove the block's own lines | **Forced by measurement.** `verify` reads `clean` across all 39 files |
| `.mjs`, `.ts` | the Go rule. For a trailing comment, delete the comment text and keep its code line | **Chosen.** Every shape reads `clean` |
| `.jsx` | the Go rule, plus §5.4's text-node caveat | **Chosen**, subject to the caveat |
| `.css` | the Go rule | **Chosen.** All three shapes read `clean` |

**Every lexable surface now has a measured row.** SQL landed at batch fifteen, `.mjs`, `.ts` and
`.jsx` at batch sixteen, and `.css` at batch seventeen.

**The rows are not all the same kind of fact, and the last column says which.** A forced row is one
`verify` decides. A chosen row is one `verify` cannot decide, where the Go shape wins on diff
cleanliness alone. On `.mjs` and `.css`, deleting the block's lines, blanking the byte range and
padding with blank lines all read `clean`, because no byte of either file reaches a consumer
(#1233, #1234, #1235, #1231). **Do not read a chosen row as proof.**

**Where a file-header block is followed by a blank line, the blank line goes with it.** That is the
merged shape from PR #1397's `github.mjs` hunk, and exactly one blank line moved in #1232's branch.
The clause binds the `.jsx`, `.mjs` and `.ts` rows.

**Every comment in `db/queries` is an own-line `--` comment.** Zero block comments, zero trailing
comments and zero glued comments across all 39 files, so #1143's `GLUE` exclusion reaches no file in
stage D2. A file-head block deletes with its trailing blank line, confirmed by #1230.
...
1. **Placement on this surface is not a readability call. An in-body SQL survivor must be
   indented.** Moving it to column 0 hoists it into the `Querier` method doc. That splits the reason
   from the clause it is about. It can also land two cited lines adjacent under ratchet rule 5. Six
   of #1227's fifteen survivors and one of #1228's six are indented accordingly.
2. **A file-head block is lifted too**, so SQL has two positions rather than three, and §4.1's
   detached-file-header route does not exist here.
3. **A comment sweep can break a Go test, and `verify` cannot see it.**
   `internal/db/observation_retention_separation_test.go` and `retention_separation_test.go` are
   hand-written and assert on the lowercased query const by substring. On `main`,
   `deleteExpiredObservations` carried the token `withdrawn` on two code lines and four comment
   lines, and #1228's sweep deleted all four. `strings.Contains(sql, "withdrawn")` passed only
   because the code lines also carry it. **Delete the last comment occurrence of a searched
   substring and the test fails.** §1.4 makes `verify` skip `internal/db`, so only `go test` catches
   this.

**`strip` refuses SQL, so a D2 ticket has no residue manifest.** All four D2 ticket bodies direct an
agent to generate one in step 1, and the command exits 2 with *"strip deletes on the Go surface
only"*. §6.4 already rules this. The ticket bodies did not, and the same wording reached all six D3
bodies.
...
| `dts-field-prose` | in a `.d.ts` file, the first code token after the block sits at brace depth 1 or deeper **and** opens a property key: an optional `readonly`, then an identifier, string or numeric key, then an optional `?`, then `:` |

**`short-label` never reaches a block this predicate claims.** The precedence order puts
`dts-field-prose` above it, so a one-line `.d.ts` field doc of six words or fewer classes as field
prose and draws no flag. That is the whole of the defect #1406 fixes. Before it, `rule.Classify`
reached `ShortLabel` for any one-liner outside a **Go** declaration position. The JS lexer sets no
`Declaration`. **The population `lint` mis-flagged was therefore exactly the population §4.3 carves
out.** #1231 and #1232 measured 167 such flags between them, 81 and 86. Every one was `short-label`
in a `.d.ts` file, and none sat outside. `Sparkline.d.ts:7` is Appendix A row 15 with verdict
**Keep**. `:4`, three lines above it and the same shape, was one of the 167.

**The predicate is a shape and not an intent guess, so ruling 12 holds.** It reads the token stream
and the file extension. It asks nothing about what the prose means, which is what separates it from
the WHY heuristic ruling 12 forbids. A block above an `export interface` sits at brace depth 0 and
is an ordinary comment (§4.3).
...
**Stage D is swept, and the cut list ran to 65 tickets rather than 64.** D1 closed at batch
fourteen, D2 at batch fifteen and D3 at batch seventeen. A 65th ticket, **#1404**, was cut
afterwards for `design-system/components/display/Card.jsx`, which the D3 cut list missed. D4 blocked
on all 65 and closed as **#1144** once they merged.

**The D3 cut list had a two-file hole, and only one file in it was work.** Of 294 tracked
`.jsx`, `.mjs`, `.ts` and `.css` files, **44** sit in no D3 ticket. After `--in-scope-only`, exactly
**two** of the 44 carry a comment. `Card.jsx` held a 152-column trailing `//` comment and was
unswept. `OrgSwitcher.d.ts` holds field prose, so §4.3 keeps it and nothing is owed. The cause
matches the Go hole's. #1139's scanner counted `/* */` blocks, every other `.jsx` block on #1232 is
`/* */`, and this was the one `//`. **`lint` could not have caught it either**, because a `.jsx`
file yields no comment block (§7.7 condition 2).
...
**The D2 and D3 rows are both wrong by construction, in opposite directions, and a ticket count must
name the population it counted.** #1139's hand scanner produced both. The D2 row counts `-- name:`
protected directives as comments: `db/queries` holds **1,492** sweepable comment lines across 39
files plus **251** directives, and the corrected figure reproduces #1143's cross-check exactly
(#1227 to #1230). The D3 row runs the other way. The scanner counts **own-line** comment lines
alone, and §3.4 puts trailing comments in scope.

| D3 ticket | Own-line | Trailing | Real |
| --- | ---: | ---: | ---: |
| #1233 | 577 | 21 | **598** |
| #1234 | 292 | 13 | **305** |
| #1235 | 359 | 53 | **412** |
...
**The orchestrator's independent re-scan reproduced all six D3 totals exactly, because it shares the
defect.** Two scanners agreeing is not corroboration when they are the same scanner twice.
...
**Two sibling tickets can hold identical prose in flight, and §4.1 cannot dedup across them.** Both
own their own files, so the cross-block test never compares them. `subjects.sql:10-17` and
`signals.sql:10` carried the same rule in two open D2 branches (#1227 to #1230). The multi-way D1
package splits priced this within one package. This is the cross-file case, and §7.5 rule 5 is the
...
7. **A batch brief is not evidence.** Rule 6 states the package-doc case, and the same failure
   ...
   **A brief is not evidence about itself either. Quote it, or do not attribute to it.** #1229's PR
   body said its spec review had corrected the brief's protected-directive count from 121 to 111.
   The brief carried no per-ticket directive count, and `grep -c 121` over it returns zero. **The
   111 is correct**, so both review axes passed the sentence: the number it lands on is true and
   only the attribution is invented. This is harder to catch than an inherited false claim, because
   the correction reads as diligence.

   **Measuring rather than complying is what catches these, and the rate is high.** Batch sixteen
   recorded four brief errors and batch seventeen three. An agent that re-measured found every one.
   Batch seventeen's were file counts. The brief said 39 and 52 `.d.ts` files where the truth is
   **52 and 49**. It said "about 130" `.jsx` comment lines for #1231 against a measured **161 across
   91 blocks**. **The orchestrator's line-scan leg is the recurring source, and a count it produces
   is a block count until proven otherwise.**
8. **Give every parallel agent a per-ticket scratchpad path.** A shared `scratchpad/gap.md` or
   ...
   **The rule held for nine batches and the harness broke it again in batch seventeen.** #1232
   overwrote a working file #1231 was reading, and one of #1231's re-measurements silently returned
   #1232's numbers. #1231 caught it on an impossible `.mjs` block count and re-derived its list from
   the ticket body. Both file lists were then re-derived from GitHub and both are intact. **A
   collision produces a plausible number, not an error**, which is why the catch has to come from a
...
| --- | --- | --- |
| Go, `.sql`, `.css`, `.mjs`, `.ts`, `.tmpl` | **Satisfiable** | The lexer yields blocks, so `lint` can flag and can reach zero |
| `.d.ts` | **Satisfiable since #1406** | It was unsatisfiable while `short-label` claimed field prose §4.3 keeps. §6.6's `dts-field-prose` row closed it, and `lint` reads zero across the tree |
| `.jsx` | **Vacuous** | The lexer yields no comment block at all |

**The `.jsx` row is the dangerous one, because a vacuous condition looks met.** `JSX.Lex` returns a
skeleton and no `Blocks` or `Trailing`, so `rule.Lint` iterates nothing and reports nothing. Across
all **141** tracked `.jsx` files the lexer yields **zero** blocks, while `.go` yields 2,204 and
`.ts` 308. `design-system/components/forms/Select.jsx` carries a 13-word `/* */` block and a 17-word
`//` line, and `lint` reads `0 violation(s)`. #1405 measured the same zero **before** its edit as
well. **A done condition that can be met without doing anything is worse than one that cannot be
met, because nothing signals it.** On a `.jsx` ticket, condition 1 and condition 3 carry the whole
gate.

**`go test` is load-bearing on stage D2 in a way it never was on D1.** §1.4 makes `verify` skip
`internal/db`, and a hand-written Go test can assert on a `db/queries` comment through the generated
query const. §6.5 measures the case. **Run the full suite on a D2 ticket and read it as a gate**,
not as a formality. A D2 PR must also ship a regenerated `internal/db`, because `sqlc` lifts a
column-0 SQL comment into the generated Go doc comment and the `sqlc` required check regenerates and
diffs.

**Where a sweep forces a code change, the code change is its own unlabelled PR and it lands first.**
A Go change inside a sweep PR correctly makes `verify` report `changed`, and `--in-scope-only` does
not help, because the forced file is in scope. `internal/commentlint/surface/corpus_test.go`'s
`TestTmplByteRangeDeleteHoldsAcrossTheCorpus` errors on any `.tmpl` file carrying no own-line
comment, and #1236 empties 23 of 24. PR #1396 first read `24 clean, 1 changed`, the only red check
of its batch. The split is sound because the change is **inert on the pre-sweep tree**: applied onto
clean `main`, the test passes, `measured` reaches 24, and the new fatal cannot fire. It landed as
**#1399**, unlabelled, because the label would arm a `verify` job on a Go change.

**`verify` is not a behaviour gate, and a ticket states its substitute when the natural suite cannot
run.** Token identity says no non-comment byte moved. It says nothing about whether the code still
works, and §5.2 records two blind spots. #1232 ran `node --test docs-site/scripts/doclint.test.mjs`
at 64 pass and 0 fail on both sides, then found that **nothing imports `measure.mjs`**, so that file
needed its own output comparison. #1231 found the docs-site suite unrunnable here, because
`astro build` needs Node 22.12.0 or newer and only v20.20.2 is installed, so it fails identically
before and after. An `esbuild --minify` comparison stood in, byte-identical on all 71 JS files for
#1231 and 49 of 49 for #1232. **Name the substitute and the files it does not reach.** §5.5 carries
the trap on that road.
...
`main` rather than in the ticket. Neither inversion recurred in batches 13 to 17.

**The risk has moved from what an agent deletes to what it writes.** Across batches thirteen,
fourteen and fifteen, **every defect either review axis found sat in a survivor the agent had just
authored, and not one sat in a deletion.** That ran unbroken for thirteen tickets. #1233 broke it,
on the first ticket whose keeper lived in a package another ticket had already swept to zero (§4.1).
Batches sixteen and seventeen then returned to the pattern for a further four tickets. **Read a
review that finds only authored defects as the expected shape**, and read one that finds an
over-delete as a signal about where the keeper lived.

**The two review axes can disagree about one line, and the disagreement is informative** (#1225).
The standards axis was right that time. This section already says the author's pass and the reviews
catch different wrong citations. It extends to the axes catching different things about one line.
Batch seventeen measured a third shape: both axes were right about different readers, and the branch
resolved it by spelling a path in full rather than by picking a winner (#1231).

**Where an author rejects a review finding on measurement, that is the process working.** #1228 kept
a line on a second valid ground after the spec axis called the deletion an over-deletion. #1229 held
...
column, which reformatted `internal/custody/egressguard_test.go` as a side effect. The equivalence
check still passed, because a token comparison does not see whitespace. §4.5's no-reformat rule is
therefore not enforced mechanically. But the diff carries a formatting change the ticket did not ask
for, and a reviewer may read it as scope creep. Say in the PR body when a hunk is pre-existing drift
rather than the ticket's own change. **#1311 is closed. PR #1440 put a `gofmt` gate in the `test`
job**, which is already a required check, so an unformatted file now blocks a merge.

**Three deletion shapes trigger the drift, not one.** The precedent above is a trailing-label
deletion merging an alignment run. #1223 hit the own-line variant: deleting a comment sitting
**between** struct-literal keys merged two runs and re-columned four untouched lines. And a
trailing-label deletion can clear pre-existing drift the ticket did not cause, which #1224 and #1225
each did to one file. Expect the effect from an own-line deletion too, and say which direction the
hunk runs.
...
**The prediction held for all eight, and batch thirteen is the first batch in which every ticket
reached it.** #1220 to #1223 and #1224 to #1226 all report `ADR gaps: none` and none filed an issue.
Read four consecutive gap-free test PRs as this SPEC working.

**A production zero is a different claim from a test zero, and it needs an audit.** Gate C does not
fire on production SQL or on a web asset. A zero there rests on §8.1's trigger never firing, which
means every rule clearing gates A and B is already written down. #1227's reviewer audited it
candidate-by-candidate, and #1230 reached the same zero. A review axis re-derived each of batch
seventeen's two zeros, from sources the author had not used. **Say in the PR body how a production
zero was reached.** A `_test.go` zero needs no such audit.
...
**The set is too narrow, and a source can be too weak to suppress a gap and still defeat it.** A
`<Component>.prompt.md`, `design-system/docs/DESIGN-NOTES.md` and `docs/agents/` are none of the five
shapes above, and each can prove a rule is written down (#1231). The five shapes decide whether a
**citation** suppresses. They do not decide whether the rule is unwritten, which is what gate A
asks. Search first, then apply the set.
...
**The search reaches the body of a `#nnn` the block itself cites.** §4.7 already warns that a token
may be defined inside an issue body. Suppression needs the same clause. #1233's per-version-search
gap was suppressed by **#354's own body**, which it had not read, because it searched `docs/` alone.
#1236 reached it from the other side: `WORK-ORDER-FEATURES-1` carries no `#nnn`, and a bare-token
issue search returns **#632**, whose body is the full rule. A `docs/`-only grep would have filed a
spurious gap.
...
§4.3 keeps `package-doc` at a 3-line cap and keeps `dts-field-prose` whole. Both are exceptions to
...
**Read every row as dated. Fourteen of the fifteen no longer point at the block they describe.**
Re-measured 2026-09-05, after all seventeen sweep batches. The earlier reading of eight was taken
after batch twelve, before the eight `cmd/web` test tickets ran:

| Rows | State |
| --- | --- |
| 2, 3, 4, 5, 6, 11, 13 | **The cited line is not the described block.** Each file was swept and merged in or before batch twelve. |
| 7, 8, 9, 10 | **Rotted at batch fourteen.** #1225 took all four. Rows 7 and 10 say *Rewrite* and #1225 **deleted** both, because `settings.go` and `handlers.go` already state those rules on `main`. A cross-`main` collapse outranks a dated worked example (§4.1), and both reviewers verified that ground independently. |
| 12 | **Rotted at batch fourteen.** #1224 took it. |
| 14 | **Rotted at batch thirteen.** #1223 deleted the `// ports census` label under the row's own verdict. |
...
| 15 | **Still resolves**, and it is the only one. `Sparkline.d.ts:7` is the `--chart-1` default above `color?: string`, and §4.3 keeps it. §6.6's `dts-field-prose` predicate is what stops `lint` flagging it. |

**Row 15 is the appendix's last resolving row, so the rules below now carry the whole of it.** Read
every other row for its reasoning and check the verdict against the file in front of you.

...
| Left a declared scope — excluded, or the scope narrowed — and nothing cites it | The **subject leaves**, taking its timelines. **No `Gap`**, because no subject is left to hold one — **and therefore one coverage-class message at the scope**, since the subject that leaves is the carrier the news would have travelled on ([#130](https://github.com/winniel123/verge-asm/issues/130) · [ADR-0074](./0074-an-aperture-narrowing-that-takes-its-carrier-with-it-fires-at-the-scope.md)) | ADR-0013 §5's table, by parity ~~on the notification question too~~ — **the parity holds for the span mechanism and NOT for the message**: ADR-0013 §5 is a table about `Custody` moves caused by **measurement**, and this row is a **Declared act** |

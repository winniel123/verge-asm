...
| Which addresses, exactly | **All 2^(32−n) of them.** Network and broadcast addresses are not exempted, because that would mean inferring the operator's subnetting, which we cannot measure and must not guess. A reserved address answers nothing and reads `not-reached`, honestly |
...
| §9's `/22` per-target range size cap | A **validation on the declaration** — §3.4's *"cannot be entered"* — operator-configurable, applied **per scope** and never to a sum |

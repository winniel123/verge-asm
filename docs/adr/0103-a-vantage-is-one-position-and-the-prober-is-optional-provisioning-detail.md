...
#188 (the measurement binary) needs a Vantage as the thing the `dns` Scan dispatches over: a **name**, a **class** (`internet`/`internal`/`unverified`) re-verified every batch, and the recursive **resolver** that [ADR-0070](./0070-a-control-probe-is-asked-from-where-the-answer-it-discriminates-was-asked-from.md) already ruled is part of the position's identity and must live on the row. Its dispatch, queue, batch, and observation tables all FK to `vantage_id`, and it ships one row — `local`, resolver-only — so a name-scope install resolves from first boot with no prober at all.

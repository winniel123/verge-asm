...
| **Give `Endpoint`'s absent `Name` an empty-name encoding** | An empty text decodes to no label sequence and is refused; the nameless `Endpoint` is a measurement mode. Collapsing them puts a refused input and a real mode in one cell. **This row rejects an encoding of the *key*.** It does not reach the wire, where a `Target`'s empty `Name` field is the absence token that the key function turns into the leading-`@` variant — [ADR-0205](./0205-an-empty-name-never-keys-a-subject-and-the-endpoint-wire-key-encodes-the-absence-of-one.md) §4 |

...
`internal/queue/crtsh.go#HTTPCTFetcher.Fetch` carried this, until [#1314](https://github.com/winniel123/verge-asm/pull/1314) deleted it:
...
The compressed survivor sits at `internal/queue/crtsh.go#NewHTTPCTFetcher`, beside the `CheckRedirect` field it
...
| `internal/delivery/runner.go#Resolver` | an operator-declared channel | Yes | [`notification-channels.md`](../spec/notification-channels.md) §4, ADR-0119 | `TestHTTPDoerRefusesRedirects` (`internal/delivery/delivery_test.go`) |
...
| `cmd/web/main.go#checkHealth` | this process's own `/healthz` over loopback | **No** | Nowhere | None |
...
Every one of the seven callers already treats a non-200 as failure. `internal/queue/crtsh.go#Worker.completeCT`
fails the job on `ferr != nil || status != http.StatusOK` and routes to `retryOrDeadLetterCT`, so no
name is admitted. `internal/release/fetcher.go#HTTPFetcher.Latest` returns an error on any non-200.
`internal/proposer/caida.go` and `internal/proposer/arin.go` do the same.
`cmd/web/main.go#checkHealth` fails the health check on a non-200. The OIDC client is handed to `go-oidc`
...
that test, at `internal/queue/crtsh_test.go#TestHTTPCTFetcherDoesNotFollowRedirect`, `internal/delivery/delivery_test.go#TestDecideRetriesFiveTimesThenDeadLetters` and
`internal/measure/httpexchange/leaf_test.go#TestTitleExtraction`. A new client owes one.
...
- **The OIDC front-channel.** [ADR-0112](./0112-single-sign-on-is-admitted-as-verified-oidc-never-header-trust.md)
  ...
  `cmd/web/handlers.go#newServer`, which performs discovery and the token exchange.
...
- **A test client.** `cmd/web/auth_test.go#newClient` sets `CheckRedirect` to inspect a 303 the console
...
- **Four clients are known violations and are not fixed here.** Each is a one-field change at a
  ...
  - `internal/release/fetcher.go#NewHTTPDoer`. **ADR-0140's Consequences already name this file as a known
  ...
  - `cmd/web/handlers.go#newServer`, the proposer registry. The ticket must confirm that no configured
    RDAP or delegated-stats endpoint answers by redirect before the change lands.
  - `cmd/web/handlers.go#newServer`, the OIDC back-channel. The client is handed to `go-oidc` and
  ...
  - `cmd/web/main.go#checkHealth`, the loopback health probe. It reaches only this process's own listener,
...
| **A `gosec` or `go vet` rule that fails an `http.Client` literal with no `CheckRedirect`** | Not decidable from the literal. `internal/release/fetcher.go#NewHTTPDoer` and `cmd/web/main.go#checkHealth` are composite literals in one line, `internal/delivery/runner.go#Resolver` sets a `Transport` built four statements earlier, and a client could equally be built by a helper and returned. A check that fires on the literal shape misses the helper and fires on a test client, which trains reviewers to suppress it. `gosec` runs `-severity high -confidence high` in CI, and this rule is neither |
...
| **Exempt the loopback health probe at `cmd/web/main.go#checkHealth`** | It reaches this process's own listener, so the exemption looks free. It is not: it makes the rule *no outbound client except where the destination is one we control*, and "a destination we control" is the judgement every one of the four violating sites would also claim. The exemption costs a re-argument at every future client and saves one line |

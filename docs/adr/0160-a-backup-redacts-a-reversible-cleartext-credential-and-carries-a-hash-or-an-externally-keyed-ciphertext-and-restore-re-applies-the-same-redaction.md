...
| `personal_token.token_hash` | a token hash | [`internal/db/models.go`](../../internal/db/models.go) |
...
([`cmd/web/restore.go`](../../cmd/web/restore.go)) calls the same function on the restore path.
...
([`cmd/web/restore.go`](../../cmd/web/restore.go)). The two redacted columns therefore land NULL,
...
- **It does not rule that the archived `account.totp_secret` is usable after a restore.** It is not.
  A restore rotates the session signing key ([`cmd/web/restore.go`](../../cmd/web/restore.go)),
  and `rotateSessionKey` re-derives the TOTP key from the new key
  ([`cmd/web/restore.go`](../../cmd/web/restore.go)). The restored ciphertext was sealed under

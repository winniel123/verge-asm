...
- The answer is the wildcard's own RRset, which is **by construction** the poison signature
  `wildcard-discrimination` measures from ~~random labels~~ **its control labels — ~~five~~ **nine** random and
  one structured, each exactly one label ([ADR-0069](./0069-a-control-label-is-one-label-and-the-set-must-falsify-label-independence.md), count raised by [#115](https://github.com/winniel123/verge-asm/issues/115))** — under ~~the apex~~ **the name's parent** —

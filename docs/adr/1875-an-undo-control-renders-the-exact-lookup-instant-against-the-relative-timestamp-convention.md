...
**The undo control on an exclusion row renders its lookup instant in full, as
`2026-08-15 09:00 UTC`. Every other console timestamp keeps the design system's terse relative
form.**

The label is a disambiguator, not a timestamp. Two declines of one scope sit side by side, and each
reverses a different proposal. `relTime` buckets whole hours under a day and whole days under a
week, so repeats of one lookup read alike. A label that cannot part two controls has failed at its
only job.

The ISO string already sat in the button's `title`. A `title` needs a mouse. Moving it into the
label reaches touch and keyboard.

Rejected: the relative form, with the `title` as the only disambiguator. It leaves the choice a
guess for anyone who cannot hover.
...
range. Those two proposals share a source, a record kind and an instant by construction. Nothing in
the three-axis label parts them.
...
```
Acme Corp · arin · RIR delegation · 2026-08-15 09:00 UTC
```

`org_name` is the registered holder the candidate arrived under. It is the only stored column that
parts two holders inside one response, and it needs no migration —
...
The `title` keeps its sentence. It now restates a fact the label already shows, which costs
nothing and keeps the hover affordance the rest of the console teaches.

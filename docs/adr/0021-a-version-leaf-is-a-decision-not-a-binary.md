...
### The ~~five~~ **six** leaves, and the one that reaches the board

> **The count is six since [#247](https://github.com/winniel123/verge-asm/issues/247) /
> [ADR-0104](./0104-an-undiscriminated-reach-is-a-gap-and-a-blanket-responder-is-measured-not-listed.md).**
> `blanket-discrimination` — the reachability twin of `wildcard-discrimination` — joins the table
> below. Amended here rather than pointed at, per
> [ADR-0058](./0058-a-superseded-mechanism-is-withdrawn-at-the-site-that-specifies-it.md), because
> *"five leaves"* read alone and present-tense would cause a session to rebuild the wrong count.
...
| `blanket-discrimination` | Is the `Address` a **blanket responder** — does it answer TCP on the control-port set? So a `reached` is trustworthy or an edge artefact | `reachability` (composed beside `connect-outcome`; an undiscriminated reach is a **`Gap`**, never a value, so this leaf is **not** in the membership vector) | Control-port count and construction — a batch-generated set of ports a well-behaved origin refuses, built to falsify port-independence as [ADR-0069](./0069-a-control-label-is-one-label-and-the-set-must-falsify-label-independence.md)'s labels falsify label-independence (the exact set is the detection ticket's, [ADR-0104](./0104-an-undiscriminated-reach-is-a-gap-and-a-blanket-responder-is-measured-not-listed.md)). **No published prefix list** — no list shapes probing (ADR-0002/0013/0089) |
...
> **This paragraph is true for three of the five leaves and false for two, and the scope is written
> here at the site that states it** ([ADR-0058](./0058-a-superseded-mechanism-is-withdrawn-at-the-site-that-specifies-it.md)).
...
> found *"one facet family clamped for one cadence"* false for **`resolution-walk`**, whose honest
> default costs **estate-wide loss of `returned`, permanently**, with the release unable to know it
> happened; [#146](https://github.com/winniel123/verge-asm/issues/146) ·
> [ADR-0086](./0086-membership-composes-every-leaf-that-decides-the-value-it-reads.md) finds it false
> for **`wildcard-discrimination`** on the same ground, that leaf being in the membership vector too.
> **So: true for `connect-outcome`, `tls-handshake` and `http-exchange`; false for `resolution-walk`
> and `wildcard-discrimination`.** For those two the excuse is removed rather than the gate changed —
> [`golden-corpus.md`](../spec/golden-corpus.md) §2 and §8 carry the pin blocks that make *we cannot
> tell* an unavailable answer, and the second direction of the gate below does the rest.

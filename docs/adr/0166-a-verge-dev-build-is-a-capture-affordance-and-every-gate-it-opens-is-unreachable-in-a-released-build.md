...
`cmd/web/adr0130_contract_test.go` already reads the flag this way: `devModeBodies` skips
...
`devSessionMint` (`cmd/web/devfixtures.go`) maps a role to a fixture username, reads the
account, and calls `completeLogin` (`cmd/web/auth.go`) directly. No password, no TOTP challenge,
no rate limiter. `seedProfileFixtures` then seeds that same account
`totp_enabled = true` with no secret (`cmd/web/devfixtures.go#seedProfileFixtures`).
...
`-seed-fixtures` (`cmd/web/main.go#main`), it exists only in a database that flag has already
...
`devSetupSeedEmpty` (`cmd/web/devfixtures.go`) runs
...
(`cmd/web/devfixtures.go`) reseeds Profile on every hit, and `devResetTOTPEnroll` clears three
...
(`cmd/web/scans.go`). Its sole reservation in Go is the comment at
`cmd/web/devfixtures.go`.
...
(`cmd/web/scans_stop_terminate_test.go#activeDispatchStore`).

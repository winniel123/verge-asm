...
`internal/queue/worker.go` carried this, until #1317 deleted it:
...
| Step | Site |
| --- | --- |
| The field, typed as a function | `internal/queue/worker.go#Worker` |
| The option that supplies it | `internal/queue/worker.go` (`WithMessages`) |
| The one production wiring site | `cmd/worker/main.go#main`, passing `delivery.EnqueueForMessage` |
...
`q.InsertDelivery` on that same handle (`internal/delivery/runner.go#EnqueueForMessage`, `:97`). It opens nothing of
...
| Site | Text |
| --- | --- |
| `internal/queue/worker.go#Worker` | `// The enqueuer is injected because internal/delivery imports this package.` |
| `internal/queue/produce.go` | `// delivery imports queue, so the reverse edge would cycle: this seam is injected, never imported.` |
| `cmd/worker/main.go#main` | `// The message hook is injected so internal/queue never imports internal/delivery.` |
...
`queue.Backoff`. `internal/report/notify.go#NotifyRunner.post` calls the same function. A session that moved
...
The rollback path is live. `internal/queue/worker.go` declares `errJobCanceled`, and the three
...
| --- | --- | --- | --- |
| `ctFetcher` | `WithCT` | **Refuses the job** | `internal/queue/crtsh.go` |
| `ctTailFetcher` | `WithCTTail` | **Refuses the job** | `internal/queue/cttail.go` |
| `ctVerifyFetcher` | `WithCTVerify` | **Refuses the call** | `internal/queue/ctverify.go` |
| `ctSource` | `WithCT` | Falls back to the keyless crt.sh path | `internal/queue/crtsh.go` |
| `router` | `WithRouter` | Probes locally | `internal/queue/worker.go` |
| `enqueue` | `WithMessages` | **Writes no message and refuses nothing** | `internal/queue/worker.go` |

`internal/queue/worker.go#Worker` states the refusing rule for the CT seams: *an unwired seam refuses
...
`cmd/worker/main.go#main` is the only caller of `NewWorker`, and `cmd/worker/main.go#main` is the only
...
The worker declares the seam as a field typed as a function (`internal/queue/worker.go#Worker`) and
...
`internal/queue/produce.go` already narrows the seam a second time inside the package, to
...
- **The schema forbids the other order.** `delivery.message_id` references `message (id)`
  (`db/migrations/20700_delivery.sql`). An enqueue in its own transaction would read a `Message`
...
`internal/queue/worker.go` returns `nil` when `produceMsgs` is false. The batch, the
...
`cmd/worker/main.go#main` wires the option, and a shipped worker that stopped messaging would be a
...
`cmd/worker/main.go#main` is the only place that names `internal/queue` and `internal/delivery`
...
- **This ADR changes no Go code.** All five limbs describe what the tree does today, verified at
  `internal/queue/worker.go`, `internal/queue/produce.go`,
  `internal/delivery/runner.go` and `cmd/worker/main.go#main`.
- **Three comments gain a citation.** `internal/queue/worker.go#Worker`, `internal/queue/produce.go`
  and `cmd/worker/main.go#main` each assert this rule and each is uncited under §4.7. All three gain
...
- **`WithMessages(nil, …)` is a defect this ruling exposes, and it ships as its own ticket.**
  ...
  refused. Production never reaches this: `cmd/worker/main.go#main` passes a real function.

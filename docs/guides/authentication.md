...
This guide covers the credential surface you manage for **your own account**. It
covers enabling two-factor, the login-time TOTP challenge, and recovering a lost
authenticator. It also covers minting and revoking API tokens, ending a session, and
changing or resetting your password. Admin acts on *other* accounts — inviting a member,
changing a role, forcing a re-enrollment — live in [accounts.md](accounts.md). Single
sign-on lives in [sso.md](sso.md).
...
1. On **Profile → Credentials**, start enrollment (`POST /account/totp/enable`). The
   screen renders a **QR code** to scan and **base32 text** for manual entry.
...
3. Enter the current 6-digit code to confirm (`POST /account/totp/confirm`). A wrong
   code re-renders the enrollment screen and does **not** enable two-factor.
4. On success, two-factor is on and **eight recovery codes** are shown **once**. Store
   them now — see [Recovery codes](#recovery-codes-and-a-lost-authenticator) below.

> **How the secret is protected.** A fresh 160-bit secret backs the QR and the base32
> text. The QR is drawn in-process, so the secret never leaves the origin. At rest the
> secret is **encrypted** with a file-backed key before it touches Postgres. A database
> dump discloses no usable secret.

An account that is **already enrolled cannot re-roll its secret** through this
path. Doing so would strip the second factor until a fresh confirm. That downgrade is
one a stolen session must not be able to perform. To rotate an authenticator you have
...
- **Replay guard.** A valid code is single-use within its own ~90-second validity
  window. The login path records the counter step it last accepted. It refuses any
  code whose step is not strictly greater. So a captured-and-replayed code cannot pass.
  Even two concurrent logins carrying the *same* code cannot both pass (RFC 6238 §5.2).
...
current authenticator stops working at once, and your next sign-in walks you through
TOTP setup again. It touches neither your password nor your session. See
...
a cookie and is never accepted on the HTML surface. The session cookie is never
...
> It coarsens to at most once per hour per token. A token that has never authenticated
> a request reads as **never**. The full endpoint reference is in **[api.md](api.md)**.
> It covers enabling the surface, the bearer header, the five read endpoints and their
> JSON shapes, and the 404-when-disabled / 405 / 401 semantics.
...
leak model — a dump of the registry yields no usable token. Because a session is
...
- **Admin** (`Settings → Sessions`). An admin sees every account's live sessions
  across the deployment, grouped by account and newest activity first. An admin can
...
A session also lapses on its own when its **12-hour** lifetime expires. Rotating the
session signing key (losing the `web-state` volume) invalidates **every** session
...
The new one is bounded to **12–72 characters** — at least 12, and bcrypt hashes no
more than 72 bytes. Your **second factor is left untouched**, so a password change
does not strip TOTP. Your **other sessions are revoked** — every other browser is
signed out through the session registry, leaving only the one that made the change.
The success notice says so (#408, ADR-0117).

### Forgot / reset

The reset flow is pre-auth — a caller who has lost their password has no session to
present — and **enumeration-safe**.
...
2. **Delivery.** A self-hosted host sends no mail, so the single-use reset link is
   ...
   record's id are written to the web logs — **not** the link. The link is a bearer
   credential that must not sit in a log (CWE-532). The operator resets on the host
   directly. Setting `VERGE_LOG_RESET_LINKS` to a non-empty value writes the plaintext
   `/reset?token=…` link into the logs. This is for an operator who has knowingly
   enabled it for their own logs.
...
revokes every session for the account through the registry. The done screen says

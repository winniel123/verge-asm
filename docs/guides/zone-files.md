...
explains what a zone file is, why verge-asm wants one, and how to get one out of the
...
A zone file is a plain-text listing of every DNS record published for a domain — the
`A`, `AAAA`, `CNAME`, `MX`, `TXT` and delegation records that make up your zone. It is
...
silence — and verge-asm can surface the removal instead of guessing. This is the one
...
(`authority: measured`), which is what lets a disagreement be *reported* rather than
one source silently overwriting the other.
...
If you run your own authoritative name servers, or your provider permits a zone
transfer from an address you control, `dig` will pull the whole zone in BIND format:
...
out of your config, or dump it (`named-compilezone`, `pdnsutil`, `kzonecheck`
...
extra cycle of slack before staleness is called.

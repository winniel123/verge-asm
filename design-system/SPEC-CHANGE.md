...
| 7 | Dashboard Vantages per-vantage latency | Build the datum — P0.5: measure RTT at the prober connect, nullable vantage column, probers.go read; spec's pending "—" until first measurement. LANDED (#485). |
...
| 11 | Report artifact severity bars + SeverityBadge column | Build — P2.10: RenderArtifact carries per-signal severity; renders when the delivery backend lands (empty-state until then is genuinely empty). LANDED (#486). |
...
| 15 | Reports "Recurring reports" — New-schedule wizard + editable schedule list | AWAITING DESIGN — surfaced by the round-2 closing gate (#487, Verify item P2.9). `examples/console/Reports.jsx` renders scheduling as a live feature (enabled "New schedule" Button → Wizard `onFinish=create`; a `SCHEDULED` list with Edit/Delete row actions). The shipped surface deliberately refuses it: `cmd/web/reports_schedule.go` returns 501 and renders the wizard disabled because there is no on-cadence dispatcher or delivery backend (#344) — a schedule row nothing reads or delivers is a declaration the product cannot honour. Options: (a) build the dispatch+delivery backend so scheduling goes live (large, was explicitly out of scope at #344); (b) change the spec to render the honest disabled/"not available yet" state, matching #344; (c) defer. Blocked until the design workspace rules. |

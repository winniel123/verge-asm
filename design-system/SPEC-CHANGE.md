...
| 16 | Report **off-instance delivery transport** — how a scheduled report reaches a recipient (`format:"email"` in Reports.jsx sample rows; test target `ops@example.com`; render.go/DocsPage call the artifact "the PDF and email rendering") | AWAITING DESIGN — surfaced building P0.6 (map #499, T1 #500). A report artifact carries the estate digest (signals/subjects/withdrawals), which **ADR-0039 §3 forbids a channel carrying off-instance** ("never the estate / no rows"); the repo has **zero SMTP** and ADR-0039 reopened SMTP only "for v1.1." So off-instance send has no ruled transport and collides with a security ADR. P0.6 builds the ADR-0039-compliant in-instance half (dispatch on cadence, store a receipt, render the artifact at the session-authed `/reports/delivery`). Options for the off-instance half: (a) an ADR-0039-compliant "a report is ready" notify-with-link (carries no estate, just a session-authed URL) — still needs an email/webhook transport; (b) a new ADR authorizing an SMTP/estate-bearing transport, overturning ADR-0039's v1.1 deferral (credential blast-radius + estate-off-instance are the costs it weighed); (c) keep reports in-instance-only, no recipient. Blocked until the design workspace rules. Tracking #497/#500. |

...
| 39 | Settings · Integrations "Send test" (P0.14, follows #38) — #38 ruling (a) directs wiring `testIntegration` to "POST a real test payload through the existing worker **to the channel URL**" with "**no new backend, handler-only**," but an integration tile has no channel URL and no binding to any Channel, so there is no target for `SendSigned` to POST to. Evidence: `catalogIntegration` (integrations.go:62-69) holds Slug/Name/Mark/Category/Description/Grants — **no URL**; `integration_state` (migration 21200) is `(slug, state)` — **no URL, no channel_id**; install collects no URL (Integrations.jsx drawer is consent/grants + Install only); Channels (migration 20600) are an independent model keyed by URL + class-routing, **never labeled by or bound to an integration** (the migration comment: "An integration is NOT a channel… never folded into channels"); and the spec's own Callout (Integrations.jsx:55) states integrations are a *future* formatting layer *on top of* channels ("Channels are built in… Integrations add formatting, acks, and state mapping on top"), matching integrations.go:24-29 (per-integration clients/credentials/formatting are "future work"). So "the channel URL" for a given tile is undefined; giving one a real target requires new backend, which #38 forbids. | **AWAITING DESIGN.** #38's ruling (a) is not implementable as scoped — its premise ("the channel URL" reachable from a tile) collides with the domain model (integration ≠ channel; no binding; no URL ever collected). Options we see, all of which lift #38's "no new backend / handler-only" constraint: **(a)** build the datum — give an integration a real endpoint (a URL + optional signing secret collected at install/config), i.e. new `integration_state` columns **and** a URL field added to the install/config drawer (spec + template change), then `testIntegration` POSTs a small signed test body via `SendSigned` to that URL; **(b)** bind to a Channel — the drawer gains a Channel selector (like report schedules bind a Channel, #17) + a nullable integration→channel binding, and the test posts through the worker to the selected Channel; **(c)** defer the affordance — retire "Send test" from the spec until a per-integration target exists (#38 foreclosed a bare retire, but if neither build path is blessed the button cannot go live and the row is a spec edit, never a silent dead handler). Constraint: every honest path needs a target the tile does not hold, so #38's "no new backend, handler-only" is itself the collision — please re-rule with the schema/spec change the chosen option requires, and re-export. |

...
| 17 | Reports "New schedule" wizard → delivery binding: #16/PARITY-CHART say "keep the spec's recipient fields" and DocsPage.jsx:99 says "schedule a recurring report **to a channel**", but the exported `Reports.jsx` wizard (Scope/Cadence/Review, lines ~128–160) captures **no recipient/channel field**, and the shipped `report_schedule.delivery_target` is free-text ("an address, a note… no channel binding yet", migration 21700). So notify-with-link (T7/#508) has **no specced destination an operator can set** — its trigger (fires on non-empty `delivery_target`) is unreachable. | **AWAITING DESIGN.** Two coupled gaps to rule: (1) **Binding model** — deliver to a **Channel** (add `report_schedule.channel_id → channel`, reuse `internal/delivery`'s signed-HTTPS webhook + `queue` backoff — ADR-0039-clean, **no new transport ADR**), or to a free-text **address** (needs a link-only mailer, which reopens the SMTP surface ADR-0039 deferred to v1.1)? (2) **Wizard UI** — add a "Delivery" step to the New-schedule wizard with the chosen destination control (a Channel selector, default "download only"); per ADR-0109 that control is a Claude Design component, not built in-repo. **Recommended:** deliver to a Channel (matches DocsPage.jsx:99); email recipients defer to v1.1. Blocks **T7/#508**; T3/T4/T5 are unaffected and T6 closes the gate on the built in-instance stack. |

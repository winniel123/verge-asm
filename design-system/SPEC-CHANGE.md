...
| 40 | RunDetail per-job live stream — design streams each running job's OWN stdout live (RunDetail.jsx LogViewer live, rundetail.tmpl .StreamHref), but the worker discards job stdout after in-process scanning; nothing persists/transports per-job output, and .Log is state-derived, not stdout | Ruling (a) SCOPED, 2026-08-28. Standing doctrine (P0.7/P0.10a/#38→P0.14): a specced region with unwired data gets built, not trimmed — so (b) reverse-spec and (c) defer are rejected. BUT option (a) as written mints a new at-rest raw-stdout store carrying estate detail, which cuts against ADR-0041 (corpus retention) and the instance's privacy posture (estate never leaves the instance; #16/#27 notify-with-link; backup secret discipline). That persistence is NOT what the spec promises — the spec promises live per-job visibility WHILE a job runs. So build the **live transport only**: the worker emits ephemeral per-job progress events over a liveness channel (queue LISTEN/NOTIFY or poll), a /runs/{id}/stream (or ?job= long-poll ?after=cursor → {lines,next,done}) endpoint serves them, and the events are **redacted exactly as the existing state-derived .Log already is**. **Persist nothing new** (no raw-stdout column/table — ADR-0041 untouched, no new privacy surface); on `done` the persisted state-derived log stands as today. "Exact output" = the job's own live progress, not raw stdout at rest. **No design/screenshot change** — LogViewer live is visually identical; only the stream's meaning (ephemeral redacted per-job events, not stored stdout) is fixed. rundetail.tmpl holes comment updated; R4-D7/#761 re-scoped. |

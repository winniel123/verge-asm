...
| 37 | P0.10 CertDetails fold (#685, AL-6): the chart says all 6 certificate rules light from the fields already on the `certificate` observation, but the CertVersion v2 leaf (internal/measure/connectoutcome/tls.go, certificateValue) parses only `not_after`, `issuer`, `algorithm`, `chain` — enough to honestly derive **2** of the 6 CertDetails booleans (`Expired`, `Expiring`, both from `not_after`). The other 4 have no source: `NotYetValid` needs the leaf's `not_before` (never captured), `SANMatchesName` needs the leaf's SANs (never captured), `WeakKeyOrSignature` needs a weak-key/signature policy (unspecified), `SelfSigned` needs the subject DN vs issuer or a chain-length rule (unspecified). CertDetails is one pointer gating all 6 rules (internal/signal/endpoint.go), so a partial construct is not honest: zero-valued `SANMatchesName=false` makes `certificate-hostname-san-mismatch` **Fire** on every named presented endpoint (a spurious critical signal), and forcing it true would silently suppress real mismatches. Both violate the "manufacture no verdict from evidence it does not have" doctrine on CertDetails and the ticket's "invent no parse and no rule." | **AWAITING DESIGN** |
| 38 | Integrations · "Send test" drawer button is a dead no-op that ships. The design spec drawer (design-system/examples/console/Integrations.jsx:64) gives each installed integration a "Send test" button that reports a delivery ("Test message sent — Check <name> for the delivery."), and settings.tmpl:1310 renders that button live (integrationsEnabled=true at cmd/web/integrations.go:30, so the surface is reachable). The handler testIntegration (cmd/web/integrations.go:298-305) validates the slug and http.Redirects back to the tab with neither a delivery nor the spec's confirmation toast — a dropped affordance (Acceptance §5) and a phantom feature. Found by the round-3 V verify gate (#689), pre-authorized by PARITY-CHART §"Verify at the round-3 gate". | **AWAITING DESIGN** — options for design: (a) wire a real integration-agnostic test POST through the existing delivery worker (raw JSON to the channel URL) plus the spec's success toast; (b) formally retire the "Send test" affordance from the spec drawer + settings.tmpl so no dead button ships; interim, the handler should at least surface the spec's toast rather than a silent redirect. Pairs with repo errata E13 ("delivery/formatting not yet built"). |

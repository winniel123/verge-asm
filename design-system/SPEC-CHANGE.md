...
| 40 | RunDetail per-job live stdout stream (R4-D7, ticket #761) — rundetail.tmpl v3.24.1 adds `.StreamHref`: a running job's OWN stdout streamed live via long-poll (`?after=cursor` → `{lines,next,done}`, auto-follow), explicitly "not a filtered slice of the batch log". But the repo has NO per-job stdout producer: the worker (`internal/queue/worker.go:44-50`) captures a job's stdout into an in-process `bytes.Buffer` consumed by the observation scanner and NEVER persists it; `queue_job` has no output/stdout/log column; RunDetail's `.Log` is synthesized from job STATE (`runLog`: kind·state·vantage·batch). | AWAITING DESIGN (2026-08-28) — a hole with no producer. **Render-match LANDED** (v3.24.1 / #768: `runView.StreamHref` nil → the static state-derived log renders; the two RunDetail render tests pass). The live stream needs a ruling: (a) build the datum — capture+persist per-job stdout in the worker (new column/table + retention) + a live transport (queue LISTEN/NOTIFY or polling) + a `/runs/{id}/stream` long-poll endpoint; (b) change the spec — the "live" view surfaces the existing state-derived job log incrementally (contradicts the design's own "not a filtered slice of the batch log", so an explicit reversal); (c) defer — `StreamHref` stays nil, static log only (what shipped). Constraint: capturing raw scan stdout at rest touches ADR-0041 (corpus retention) + privacy (stdout may carry estate detail); worker→web liveness needs a transport decision. |

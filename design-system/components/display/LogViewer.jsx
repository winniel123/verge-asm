...
export function LogViewer({ lines = [], title, live, height = 220, style }) {
  ...
  return (
    <div style={{ background: "var(--console-surface)", borderRadius: 12, overflow: "hidden", ...style }}>
      {(title || live) && (
      ...
      <div ref={ref} style={{ height, overflowY: "auto", overflowX: "hidden", padding: "10px 14px 12px" }}>
        {lines.map((l, i) => (
          <div key={i} style={{ display: "flex", gap: 10, font: "400 11.5px/1.7 var(--font-mono)" }}>
            <span style={{ color: "var(--neutral-500)", flex: "none" }}>{l.time}</span>
            <span style={{ color: LC[l.level] || "var(--console-text)", overflowWrap: "anywhere" }}>{l.text}</span>
          </div>
        ))}
      </div>
    </div>

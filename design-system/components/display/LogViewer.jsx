...
/* Live batch output on inverted ink. lines: [{time, level?, text}]. Auto-follows while live. */
const LC = { info: "var(--neutral-400)", warn: "#e0aa4a", error: "#f08c82" };
export function LogViewer({ lines = [], title, live, height = 220, style }) {
  ...
  return (
    <div style={{ background: "var(--surface-inverted)", borderRadius: 12, overflow: "hidden", ...style }}>
      {(title || live) && (
      ...
      <div ref={ref} style={{ height, overflowY: "auto", overflowX: "hidden", padding: "10px 14px 12px" }}>
        {lines.map((l, i) => (
          <div key={i} style={{ display: "flex", gap: 10, font: "400 11.5px/1.7 var(--font-mono)" }}>
            ...
            <span style={{ color: LC[l.level] || "var(--text-on-inverted)", overflowWrap: "anywhere" }}>{l.text}</span>
          </div>
        ))}
      </div>
    </div>

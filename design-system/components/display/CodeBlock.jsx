...
export function CodeBlock({ children, title, copyText, style }) {
  ...
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{ position: "relative", background: "var(--console-surface)", borderRadius: 12, overflow: "hidden", ...style }}>
      ...
      <pre ref={preRef} style={{ margin: 0, padding: "13px 16px", overflow: "auto" }}>
        <code style={{ font: "400 12.5px/1.6 var(--font-mono)", color: "var(--console-text)" }}>{children}</code>
      </pre>
      {children != null && (

...
- **R4-D7 Job streams exact output** (RunDetail) — re-scoped by collision #40 (ruled a-scoped). Build the **live transport only**: worker emits ephemeral per-job progress events (LISTEN/NOTIFY or poll), a ?job= long-poll (?after=cursor → {lines,next,done}) serves them, redacted exactly as the state-derived .Log. **Persist nothing new** — no raw-stdout store (ADR-0041/privacy); on done the state-derived log stands. StreamHref hole + transport landed in rundetail.tmpl/RunDetail.jsx. Repo wires the worker event emit + endpoint.

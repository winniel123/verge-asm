...
| AL-10 | 8 integration tiles: install state real, downstream effect missing; "Send test" no-op | RULED + CHARTED | #26j catalogue-PRG ruling (label fix = E13); "Send test" no-op ruled by #38 → build (a), P0.14 (wire through the integration-agnostic worker; spec toast stands). No longer a dead button once P0.14 lands |

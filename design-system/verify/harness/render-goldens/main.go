...
func main() {
	screen := flag.String("screen", "inventory", "which screen to render: inventory | error | profile | signin | setup | coverage | exposure")
	...
	default:
		log.Fatalf("render-goldens: unknown -screen %q (want inventory | error | profile | signin | setup | coverage | exposure)", *screen)
	}
}

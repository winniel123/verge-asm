...
echo "== 1. render-goldens -> static HTML (inventory + error + profile + signin + setup + coverage + exposure) =="
...
if [ "${GOLDENS:-}" = "write" ]; then
  echo "== 2. capture --write-goldens (file://) — inventory + error + profile + signin + setup + coverage + exposure =="
...
echo "== 4. capture --mode candidate ${ADV_FLAG} — inventory + error + profile + signin + coverage + exposure + setup =="

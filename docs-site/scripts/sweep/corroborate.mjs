import { spellsCommitHash, spellsPath } from "../citations/extract.mjs";
...
function reads(body, token, env) {
  ...
  if (spellsCommitHash(body)) return false;
...
export function rivalName(lineText, token, names, region, env) {
  ...
  for (const { ident, before: leads } of idents) {
    const rival = rivalFor(ident, declared, region);
    if (rival !== null) (leads ? before : after).add(rival);
  }

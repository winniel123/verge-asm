...
import { CONTAINMENT_ROW } from "./rows/containment.mjs";

// A row keys on a path predicate, because the predicate follows the tool that owns the
// file. A bare extension does not.
export const ROWS = [GO_ROW, SQLC_ROW, TEMPLATE_ROW, MARKDOWN_ROW, CONTAINMENT_ROW];

// A declared name is MUST and containment is MAY, so a declaring row wins (SPEC §3.2 rule 4).
export function rowFor(path, rows = ROWS) {
  return rows.find((row) => row.matches(path)) ?? CONTAINMENT_ROW;
}

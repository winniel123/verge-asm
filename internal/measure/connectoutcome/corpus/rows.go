...
var Rows = []Row{
...

	{
		...
		Claim:        "a scope whose only pairs are UDP produces no reachability observation at all — a UDP pair is a member of the port set and travels in the scope record, but is never probed, so no Service exists for it (v1 spec §3.5, ADR-0083)",
		SpecVerified: true,
		Profile:      profile(),
		Step: Step{
			Batch:   "b1",

...
type SafetyProfile struct {
	...
	// argument holds at one vantage. The `hot` Scan builds one job per
	// `(Vantage, Address)` pair. Two vantages therefore put one host in two jobs,
	// and a second worker can double the real rate against it.

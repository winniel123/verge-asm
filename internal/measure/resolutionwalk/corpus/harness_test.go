...
// parameter change, having bumped resolutionwalk.Version, and commit the result.
...
func regenerate() error {
	...
	return WriteLock(".", Lock{
		LeafVersion:    resolutionwalk.Version,
...
// A5: coverage. Every cell of the block holds at least one row, and no row cites
// a cell outside the enumeration. A missing cell fails the build, naming it.
func TestCorpusCoverage(t *testing.T) {
	...
	if len(AllCells) != 27 {
		t.Errorf("resolution-walk block is 27 cells (golden-corpus.md §2.5); AllCells has %d", len(AllCells))
	}
}

// The lock gate: recomputed digests must equal the checked-in lock, and the
// lock's leaf version must equal the code's. Any output or parameter change
// forces a lock edit; binding that edit to a version bump is A6's job in CI
...
func TestCorpusLock(t *testing.T) {
	...
	if got := CorpusDigest(rendered); got != lock.CorpusDigest {
		t.Errorf("corpus digest moved: a row's expected output changed.\n  locked: %s\n  now:    %s\nBump resolutionwalk.Version and re-bless with -update (or the change is unintended).",
			lock.CorpusDigest, got)
	}
	if got := ParamsDigest(); got != lock.ParamsDigest {
		t.Errorf("params digest moved: a declared offer changed.\n  locked: %s\n  now:    %s\nBump resolutionwalk.Version and re-bless with -update.",

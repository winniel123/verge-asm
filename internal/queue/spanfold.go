...
func foldObservationsIntoSpans(ctx context.Context, qtx *db.Queries, batchID int64, vantageID pgtype.Int8, observedAt time.Time, obs []wire.Observation, in membershipInputs) error {
	for _, o := range obs {
		if o.Facet == "" {
			continue
		}
		if err := foldOne(ctx, qtx, batchID, vantageID, observedAt, o, in); err != nil {
			return err
		}
	}
	return nil
...
func foldOne(ctx context.Context, qtx *db.Queries, batchID int64, vantageID pgtype.Int8, observedAt time.Time, o wire.Observation, in membershipInputs) error {

	...
	if !HotLagGateArmed(staleJobThreshold) {
		if logger != nil {
			logger.Printf("dispatcher: the stale-running reaper is disabled (stale job timeout %s), so the cadence-lag gate is not armed; "+
				...
				"the hot tier opens later; a re-point residue names that ground instead, so a re-point onto an address new to the estate is "+
				"announced across two messages; the residue is still read on the next poll pass rather than on a drained tier, so it names no "+

...
In a migration comment, name a goose annotation and never spell it. goose reads any comment line holding `+goose` as an annotation, quoted or not, and refuses the file. That refusal exits the same `web` binary through `log.Fatalf`, so the container restarts instead of serving; there is no `panic:` line to grep for (#2261). `TestEveryMigrationParsesUnderGoose` and `TestNoMigrationSpellsABehaviourChangingAnnotation` in the required `test` job catch it before CI does. See `docs/spec/comment-policy.md` §2.3 for the one case they miss.

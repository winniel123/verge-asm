...
func (h *progressHub) record(ev jobProgress) {
	...
	// Evicting the front would shift every later index and desync the stream cursor (ADR-0182 §2).
...
func eventStreamLines(events []jobProgress, jobFilter int64, filtered bool) []runStreamLine {
	// The viewer only appends, so a reason merged onto an existing line never lands (ADR-0182 §1).
...
func encodeStreamCursor(events, state int) int {
	// A retry growing the state log must not shift an event's position (ADR-0182 §2).

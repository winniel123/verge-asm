...
func (l *loginLimiter) locked(keys ...string) bool {
	...
	for _, k := range keys {
		...
		// A capped lock stops a pre-auth attacker locking a username out for good (ADR-0170 §4).
...
func (l *loginLimiter) fail(keys ...string) (nowLocked bool) {
	...
	for _, k := range keys {
		...
		if e.failures >= l.maxFailures {
			// Anchoring to the first lock stops re-locks sliding the ceiling (ADR-0170 §3).

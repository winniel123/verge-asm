  ...
  return (
    <div style={{ display: "inline-flex", flexDirection: "column", gap: 8, fontFamily: "var(--font-ui)", ...style }}>
      <div role="img" aria-label={label} style={{ display: "grid", gridTemplateRows: "repeat(7, " + cell + "px)", gridAutoFlow: "column", gridAutoColumns: cell + "px", gap }}>
        {values.map((v, i) => (
          <span key={i} title={v + " " + unit} style={{ width: cell, height: cell, borderRadius: 3, background: bg(level(v)), border: "1px solid " + (level(v) === 0 ? "var(--row-sep)" : "transparent"), boxSizing: "border-box", opacity: on ? 1 : 0, transition: "opacity 320ms var(--ease-out)", transitionDelay: Math.min(Math.floor(i / 7) * 12, 700) + "ms" }} />
        ))}
      </div>
      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
        ...
        {[0, 1, 2, 3, 4].map((l) => <span key={l} style={{ width: 10, height: 10, borderRadius: 3, background: bg(l), border: "1px solid " + (l === 0 ? "var(--row-sep)" : "transparent"), boxSizing: "border-box" }} />)}

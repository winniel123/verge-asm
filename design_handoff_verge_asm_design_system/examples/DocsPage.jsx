...
import { Accordion } from "../../components/display/Accordion.jsx";
...
const NAV = [
  ...
  ["Scanning", ["Seeds", "Vantages", "Schedules"]],
  ["Signals", ["Severity levels", "Annotations", "Channels"]],
  ["Reference", ["Components", "API", "CLI", "Self-hosting"]],
];
...
export function DocsPage() {
  ...
  return (
    <div data-screen-label="Docs" style={{ background: "var(--bg-page)", minHeight: "100vh", fontFamily: "var(--font-ui)" }}>
      <nav style={{ display: "flex", alignItems: "center", gap: 16, height: 56, padding: "0 24px", background: "var(--surface)", borderBottom: "1px solid var(--border-default)" }}>
        ...
        <Input size="sm" mono placeholder="Search docs" prefix={<Icon name="search" size={13} />} style={{ marginLeft: "auto", width: 260 }} />
        <Kbd keys={["mod", "K"]} />
        <VersionSelect value={ver} onChange={setVer} versions={[{ value: "v0.9.2", tag: "current" }, { value: "v0.9.1" }, { value: "v0.8.7" }, { value: "main", tag: "dev" }]} />
        <a href="#" style={{ font: "500 12px var(--font-ui)", color: "var(--text-secondary)", textDecoration: "none" }}>GitHub</a>
      </nav>
      <div style={{ maxWidth: 1240, margin: "0 auto", display: "grid", gridTemplateColumns: "220px minmax(0, 1fr) 180px", gap: 48, padding: "40px 32px 80px" }}>
        <aside style={{ display: "flex", flexDirection: "column", gap: 24, position: "sticky", top: 40, alignSelf: "start" }}>
          {NAV.map(([section, pages]) => (
            <div key={section} style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              ...
              {pages.map((p) => {
                const active = p === "Quick start";
                return <a key={p} href={p === "Components" ? "./reference.html" : "#"} style={{ font: (active ? "600" : "400") + " 13px var(--font-ui)", color: active ? "var(--link)" : "var(--text-secondary)", textDecoration: "none", padding: "5px 10px", borderRadius: 8, background: active ? "var(--accent-soft)" : "transparent" }}>{p}</a>;
              })}
            </div>
          ))}
        </aside>
        ...
        <aside style={{ position: "sticky", top: 40, alignSelf: "start", display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          {["Run the container", "Add your first seed", "Severity levels", "Common questions"].map((t, i) => (
            <a key={t} href="#" style={{ font: "400 12.5px var(--font-ui)", color: i === 0 ? "var(--link)" : "var(--text-secondary)", textDecoration: "none" }}>{t}</a>
          ))}
        </aside>
      </div>
    </div>
